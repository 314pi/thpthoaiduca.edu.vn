	...
	if(!empty($cats)){
		for ($i=0; $i < $limit; $i++) { 	
           if ($cats[$i]->cat_title){
               ...
               if ($user->get('aid') >= $access[0] || $cats[$i]->cat_access == '99') {
        ...
        $app->appendMetaTag( 'keywords' , strip_tags($metakey));  
...
function showOneCategory($option, $cat, $subcats, $files, $catid, $total, $sum_pages, $limit, $limitstart, $sum_subcats, $sum_subfiles, $site_aktuell, $access, $columns, $pageNav, $order, $dir){
    ...
    if ($site_aktuell > 1) $subcats = '';
        ...
        if ($viewcatinfo) {
		    ...
		    $html_cat = str_replace('{cat_pic}', $catpic, $html_cat);
            $html_cat = str_replace('{sum_subcats}', '', $html_cat);
            $html_cat = str_replace('{sum_files_cat}', JText::_('JLIST_FRONTEND_COUNT_FILES').' '.$total, $html_cat);
            $html_cat = str_replace('{cat_info_begin}', '', $html_cat); 
            $html_cat = str_replace('{cat_info_end}', '', $html_cat);
        ...
        for ($i=0; $i<count($files); $i++) {
            ...
            if ($i > 0){
             ...
             if ($jlistConfig['use.report.download.link']){
                $report_link = '<a href="'.JRoute::_("index.php?option=com_jdownloads&amp;Itemid=".$cat_itemid."&amp;view=report&amp;cid=".$files[$i]->file_id).'">'.JText::_('JLIST_CONFIG_REPORT_FILE_LINK_TEXT').'</a>';
            ...
            if ( ($user->get('aid') >= $access[1] || $access[1] == '9') && $jlistTemplates[2][0]->checkbox_off == 0 ) {
            ...
            if ($user->get('aid') >= $access[1] || $access[1] == '9') {
            ...
            if ($jlistConfig['view.detailsite']){
                ...
                if ($user_can_seen_download_url){
                   ...
                   if ($url_task == 'finish'){ 
                      $download_link_text = '<a '.$blank_window.' href="'.$download_link.'" title="'.JText::_('JLIST_LINKTEXT_DOWNLOAD_URL').'" class="jd_download_url">'.$files[$i]->file_title.'</a>';
                   } else {
                      $download_link_text = '<a href="'.$download_link.'" title="'.JText::_('JLIST_LINKTEXT_DOWNLOAD_URL').'">'.$files[$i]->file_title.'</a>';                  
                   }
                   // Symbol anzeigen - auch als url
                   if ($files[$i]->file_pic != '' ) {
                        $fpicsize = $jlistConfig['file.pic.size'];
        ...
        if ($user->get('aid') >= $access[1] || $access[1] == '9') {
            ...
            if ($access[1] == 2){
                 ...
                 $regg = str_replace('<br />', '', '<b>'.JText::_('JLIST_FRONTEND_CAT_ACCESS_REGGED').' '.JText::_('JLIST_FRONTEND_CAT_ACCESS_REGGED2').'</b>');
            } else {
                 $regg = str_replace('<br />', '', '<b>'.JText::_('JLIST_FRONTEND_CAT_ACCESS_REGGED').'</b>');
            }
            $html_cat = str_replace('{checkbox_top}', '<div style="text-align:center; padding:8px;"><img src="'.JURI::base().'components/com_jdownloads/images/info32.png" align="middle" width="32" height="32" border="0" alt="" /> '.$regg.'</div>', $html_cat);                    
        }
        ...
        if (($user->get('aid') >= $access[1] || $access[1] == '9') && $jlistTemplates[2][0]->checkbox_off == 0 && !empty($files)) {
...
function showDownload($option, $file, $cat, $access, $edit_link, $user_can_edit){
    ...
    if ((int)$jlistConfig['use.tabs.type'] > 0){
       ...
       $html_file = str_replace('{tab description}', $jd_pane->startPanel(JText::_('JLIST_FE_TAB_DESCRIPTION_TITLE'), 'panel1'), $html_file);
       $html_file = str_replace('{tab description end}', $jd_pane->endPanel(JText::_('JLIST_FE_TAB_DESCRIPTION_TITLE'), 'panel1'), $html_file);
       $html_file = str_replace('{tab pics}', $jd_pane->startPanel(JText::_('JLIST_FE_TAB_PICS_TITLE'), 'panel2'), $html_file);
       $html_file = str_replace('{tab pics end}', $jd_pane->endPanel(JText::_('JLIST_FE_TAB_PICS_TITLE'), 'panel2'), $html_file);
       $html_file = str_replace('{tab mp3}', $jd_pane->startPanel(JText::_('JLIST_FE_TAB_AUDIO_TITLE'), 'panel3'), $html_file);
       $html_file = str_replace('{tab mp3 end}', $jd_pane->endPanel(JText::_('JLIST_FE_TAB_AUDIO_TITLE'), 'panel3'), $html_file);
       $html_file = str_replace('{tab data}', $jd_pane->startPanel(JText::_('JLIST_FE_TAB_DATA_TITLE'), 'panel4'), $html_file);
       $html_file = str_replace('{tab data end}', $jd_pane->endPanel(JText::_('JLIST_FE_TAB_DATA_TITLE'), 'panel4'), $html_file);
       $html_file = str_replace('{tab download}', $jd_pane->startPanel(JText::_('JLIST_FE_TAB_DOWNLOAD_TITLE'), 'panel5'), $html_file);
       $html_file = str_replace('{tab download end}', $jd_pane->endPanel(JText::_('JLIST_FE_TAB_DOWNLOAD_TITLE'), 'panel5'), $html_file);
    ...
    if ($file->release) {
        $html_file = str_replace('{release}',JText::_('JLIST_FRONTEND_VERSION_TITLE').$file->release.$edit_link, $html_file);        
    } else {
    ...
    if ($user->get('aid') >= $access[1] || $access[1] == '9') {
        ...
        if ($access[1] == 2){
            // additional msg: download only for special members
            $regg = str_replace('<br />', '', JText::_('JLIST_FRONTEND_CAT_ACCESS_REGGED').' '.JText::_('JLIST_FRONTEND_CAT_ACCESS_REGGED2'));
        } else {
...
function Summary($option, $marked_files_id, $mail_files, $filename, $download_link, $del_ok, $extern_site, $sum_aup_points, $has_licenses, $open_in_blank_page, $must_confirm, $license_text, $zip_file_info, $file_title){
    ...
    $mainframe->setPageTitle($page_title.' - '.JText::_('JLIST_BACKEND_TEMP_TYP3').htmlspecialchars($file_title) ); 
    ...
    if (count($marked_files_id) > 1) {
        // mass download
         if ($must_confirm){
            ...
            $agree_form .= '<input type="checkbox" name="license_agree" onclick="enableDownloadButton(this)" /> '.JText::_('JLIST_FE_SUMMARY_LICENSE_VIEW_AGREE_TEXT').'<br /><br />';
            $agree_form .= '<input type="submit" name="submit" id="jd_license_submit" class="button" value="'.JText::_('JLIST_LINKTEXT_DOWNLOAD_URL').'" disabled="disabled" />';
        ...
        if ($must_confirm){
            if ($license_text != ''){
                $html_sum = str_replace('{license_title}', JText::_('JLIST_FE_SUMMARY_LICENSE_VIEW_TITLE'), $html_sum);
...
function viewUpload($option, $view ){
    ...
    if (intval($jlistConfig['upload.access.group']) > 0){ 
        ...
        if (in_array($jlistConfig['upload.access.group'], $user_groups) || ($user->usertype == 'Super Administrator' || $user->usertype == 'Administrator')){
            $can_upload = true;
        } else {
        ...
        if ($user->get('aid') < (int)$jlistConfig['upload.access']){
    ...
    $error_msg = str_replace('<br />', '\n', JText::_('JLIST_FRONTEND_UPLOAD_ERROR_NO_VALUE'));
    $error_msg_ext = JText::_('JLIST_FRONTEND_UPLOAD_ERROR_WRONG_FILE_EXT');
    ...
    $form = '<form name="uploadForm" id="uploadForm" action="'.$uri->_uri.'" onsubmit="return CheckForm(\''.$error_msg.'\', \''.$allowed_file_types.'\', \''.$error_msg_ext.'\');" method="post" enctype="multipart/form-data">
	<table class="jd_upload_form" border="0" cellpadding="0" cellspacing="5" width="99%">
      ...
      if ($jlistConfig['fe.upload.view.price'] == '1') {     
        ...
        if ($jlistConfig['fe.upload.view.select.file'] == '1') {
            ...
            <td width="200">
                <input name="file_upload" id="file_upload" size="30" type="file" onchange="'.$checkUpload.'" value="'.$file_upload.'"/><br />'.JText::_('JLIST_FRONTEND_UPLOAD_ALLOWED_FILETYPE').': '.$allowed_file_types_view.'<br />'.JText::_('JLIST_FRONTEND_UPLOAD_ALLOWED_MAX_SIZE').': '.$jlistConfig['allowed.upload.file.size'].' KB
            </td>
            </tr>';
        }
        if ($jlistConfig['fe.upload.view.extern.file'] == '1') {
        ...
        if ($jlistConfig['fe.upload.view.pic.upload'] == '1') { 
            ...
            <td width="200">
                <input name="pic_upload" size="30" type="file" value="'.$pic_upload.'"/><br />'.JText::_('JLIST_FRONTEND_UPLOAD_PIC_ALLOWED_FILES').' gif, jpg, png
            </td>
            </tr>';
            $form .= '<tr>
            <td width="140" valign="top">'.JText::_('JLIST_FRONTEND_UPLOAD_PIC_FILETITLE2').'
            </td><td width="20" valign="top">&nbsp;</td>
            <td width="200">
                <input name="pic_upload2" size="30" type="file" value="'.$pic_upload2.'"/><br />'.JText::_('JLIST_FRONTEND_UPLOAD_PIC_ALLOWED_FILES').' gif, jpg, png
            </td>
            </tr>';
            $form .= '<tr>
            <td width="140" valign="top">'.JText::_('JLIST_FRONTEND_UPLOAD_PIC_FILETITLE2').'
            ...
            <td width="200">
                <input name="pic_upload3" size="30" type="file" value="'.$pic_upload3.'"/><br />'.JText::_('JLIST_FRONTEND_UPLOAD_PIC_ALLOWED_FILES').' gif, jpg, png
            </td>
            </tr>';
		}  
        if ($jlistConfig['fe.upload.view.desc.short'] == '1') {
...
function showSearchForm($option){
    ...
    $html_form = '<form name="jdsearch" action="'.JRoute::_('index.php?option=com_jdownloads&amp;Itemid='.$Itemid.'&amp;view=searchresult').'" onsubmit="return CheckSearch(\''.JText::_('JLIST_FRONTEND_SEARCH_RESULT_TEXT_TO_SHORT').'\',\''.JText::_('JLIST_FRONTEND_SEARCH_RESULT_NO_OPTION').'\');" method="post">';
