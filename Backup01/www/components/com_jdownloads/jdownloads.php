    ...
    for ($i=0; $i<count($mail_files_arr); $i++) {
       ...
       if ($mail_files_arr[$i]->license > 0){  
           ...
           if ($license->license_url){
               ...
               $mail_files .= "<div><li><b>".$mail_files_arr[$i]->file_title.' '.$mail_files_arr[$i]->release.'&nbsp;&nbsp;&nbsp;</b>'.JText::_('JLIST_FE_DETAILS_LICENSE_TITLE').': <b><a href="'.$license->license_url.'" target="_blank">'.$license->license_title.'</a> '.$extern_url_pic.' &nbsp;&nbsp;&nbsp;</b>'.JText::_('JLIST_FE_DETAILS_FILESIZE_TITLE').': <b>'.$mail_files_arr[$i]->size.'</b></li></div>';
           } else {
               $mail_files .= "<div><li><b>".$mail_files_arr[$i]->file_title.' '.$mail_files_arr[$i]->release.'&nbsp;&nbsp;&nbsp;</b>'.JText::_('JLIST_FE_DETAILS_LICENSE_TITLE').': <b>'.$license->license_title.'&nbsp;&nbsp;&nbsp;</b>'.JText::_('JLIST_FE_DETAILS_FILESIZE_TITLE').': <b>'.$mail_files_arr[$i]->size.'</b></li></div>';
           }   
       } else {
   ...
   if ($jlistConfig['use.blocking.list'] && $jlistConfig['blocking.list'] != '') {
    ...
    if ($files){
        ...
        foreach ($files_arr as $file){
            $may_download = checkLog($file, $user, $can_download);
        }    
        if (!$may_download){
        ...
        // check the log - can user download the file?
        $may_download = false;
        $may_download = checkLog($fileid, $user, $can_download);
        if (!$may_download){
            // download not possible
            $datenow =& JFactory::getDate(); 
            $date = $datenow->toFormat("%Y-%m-%d %H:%m");
...
function showOneCategory($option, $cid) {
    ...
    $user = &JFactory::getUser();
    $limit_cats = intval($jlistConfig['categories.per.side']); 
...
function showDownload($option,$cid){
  ...
  // $access = checkAccess_JD();
   ...
   if ($user->id > 0){
      if (($user->id == $file->submitted_by && $jlistConfig['uploader.can.edit.fe'] == '1') || ($user->usertype == 'Super Administrator' || $user->usertype == 'Administrator')){
          $user_can_edit = true;
      } else {   
   ...
   if ($user_can_edit){
       ...
       $edit_pic = '<img src="'.JURI::base().'components/com_jdownloads/images/edit.png" title="'.JText::_('JLIST_BACKEND_TOOLBAR_EDIT').'" alt="'.JText::_('JLIST_BACKEND_TOOLBAR_EDIT').'" />';   
...
function makeHeader($header, $compo_text, $is_showcats, $is_one_cat, $sum_subs, $is_detail, $is_search, $is_upload, $is_summary,  $is_finish, $sum_pages, $limit, $total, $limitstart, $site_aktuell, $pageNav, $order, $dir) {
    ...
    // home link
    $home_link = '<a href="'.JRoute::_('index.php?option=com_jdownloads&amp;Itemid='.$root_itemid).'">'.'<img src="'.JURI::base().'components/com_jdownloads/images/home_fe.png" width="32" height="32" border="0" alt="" /></a> <a href="'.JRoute::_('index.php?option=com_jdownloads&amp;Itemid='.$root_itemid).'">'.JText::_('JLIST_FRONTEND_HOME_LINKTEXT').'</a>';
    // insert search link
    $search_link = '<a href="'.JRoute::_('index.php?option=com_jdownloads&amp;Itemid='.$search_link_itemid.'&amp;view=search').'">'.'<img src="'.JURI::base().'components/com_jdownloads/images/search.png" width="32" height="32" border="0" alt="" /></a> <a href="'.JRoute::_('index.php?option=com_jdownloads&amp;Itemid='.$search_link_itemid.'&amp;view=search').'">'.JText::_('JLIST_FRONTEND_SEARCH_LINKTEXT').'</a>';
    // insert frontend upload link if active
    $upload_link = '<a href="'.JRoute::_('index.php?option=com_jdownloads&amp;Itemid='.$upload_link_itemid.'&amp;view=upload').'">'.'<img src="'.JURI::base().'components/com_jdownloads/images/upload.png" width="32" height="32" border="0" alt="" /></a> <a href="'.JRoute::_('index.php?option=com_jdownloads&amp;Itemid='.$upload_link_itemid.'&amp;view=upload').'">'.JText::_('JLIST_FRONTEND_UPLOAD_LINKTEXT').'</a>';
    ...
    if ($is_detail){
            ...
            $header = str_replace('{upper_link}', '<a href="'.$upper_url.'">'.'<img src="'.JURI::base().'components/com_jdownloads/images/upper.png" width="32" height="32" border="0" alt="" /></a> <a href="'.$upper_url.'">'.JText::_('JLIST_FRONTEND_HEADER_UPPER_LINKTEXT').'</a>', $header);
    } else {   
    ...
    if ($jlistConfig['show.header.catlist']){
		...
		$query = "SELECT cat_id AS id, parent_id AS parent, cat_title AS name FROM #__jdownloads_cats WHERE published = 1 AND (cat_access <= '$access' $user_groups) ORDER BY $cat_sort_field $cat_sort";
		$database->setQuery( $query );
		$src_list = $database->loadObjectList();
        $query = "SELECT cat_id AS id, parent_id AS parent, cat_title AS name FROM #__jdownloads_cats WHERE published = 1 AND (cat_access <= '$access' $user_groups) ORDER BY cat_id";
        $database->setQuery( $query );
    ...
    if ($jlistConfig['view.subheader']) {
        if ($is_showcats) {
           if ($jlistConfig['option.navigate.top']){
                ...
                $subheader = str_replace('{subheader_title}', JText::_('JLIST_FRONTEND_SUBTITLE_OVER_CATLIST'), $subheader);
                ...
                $subheader = str_replace('{count_of_sub_categories}', JText::_('JLIST_FRONTEND_SUBHEADER_NUMBER_OF_CATS_TITLE').': '.$total, $subheader);
           } else {
                $subheader = str_replace('{subheader_title}', JText::_('JLIST_FRONTEND_SUBTITLE_OVER_CATLIST'), $subheader);
        ...
        if ($is_one_cat) {
           ...
           if ($jlistConfig['option.navigate.top']){
                ...
                $subheader = str_replace('{subheader_title}', JText::_('JLIST_FRONTEND_SUBTITLE_OVER_ONE_CAT').': '.$titles, $subheader);
                $subheader = str_replace('{page_navigation}', $page_navi_links, $subheader);
                $subheader = str_replace('{page_navigation_results_counter}', $page_navi_counter, $subheader);
                $subheader = str_replace('{page_navigation_pages_counter}', $page_navi_pages, $subheader);                
           ...
           if ($jlistConfig['view.sort.order'] && $total > 1){
               if($order == 'default' || $order == ''){
                   $sort_default = JText::_('JLIST_FE_SORT_ORDER_DEFAULT').' | ';
               } else {
                   $sort_default = '<a href="'.JRoute::_('index.php?option=com_jdownloads&amp;Itemid='.$Itemid.'&amp;view=viewcategory&amp;catid='.$catid.'&amp;limitstart='.$limitstart.'&amp;order=default&amp;dir=asc').'">'.JText::_('JLIST_FE_SORT_ORDER_DEFAULT').'</a> | ';                   
               }
               if($order != 'name'){
                  $sort_name = '<a href="'.JRoute::_('index.php?option=com_jdownloads&amp;Itemid='.$Itemid.'&amp;view=viewcategory&amp;catid='.$catid.'&amp;limitstart='.$limitstart.'&amp;order=name&amp;dir=asc').'">'.JText::_('JLIST_FE_SORT_ORDER_NAME').'</a> | ';
               } else {
                  $sort_name = JText::_('JLIST_FE_SORT_ORDER_NAME').' | ';
               }   
               if($order != 'date'){
                   $sort_date = '<a href="'.JRoute::_('index.php?option=com_jdownloads&amp;Itemid='.$Itemid.'&amp;view=viewcategory&amp;catid='.$catid.'&amp;limitstart='.$limitstart.'&amp;order=date&amp;dir=asc').'">'.JText::_('JLIST_FE_SORT_ORDER_DATE').'</a> | ';
               } else {
                  $sort_date = JText::_('JLIST_FE_SORT_ORDER_DATE').' | ';
               }   
               if($order != 'hits'){
                   $sort_hits = '<a href="'.JRoute::_('index.php?option=com_jdownloads&amp;Itemid='.$Itemid.'&amp;view=viewcategory&amp;catid='.$catid.'&amp;limitstart='.$limitstart.'&amp;order=hits&amp;dir=asc').'">'.JText::_('JLIST_FE_SORT_ORDER_HITS').'</a> | ';
               } else {
                  $sort_hits = JText::_('JLIST_FE_SORT_ORDER_HITS').' | ';
               }
                    
               if ($dir == 'asc' || $dir == ''){
                   $sort_direction = '<a href="'.JRoute::_('index.php?option=com_jdownloads&amp;Itemid='.$Itemid.'&amp;view=viewcategory&amp;catid='.$catid.'&amp;limitstart='.$limitstart.'&amp;order='.$order.'&amp;dir=desc').'">['.JText::_('JLIST_FE_SORT_ORDER_DESC').'</a>]';
               } else{
                   $sort_direction = '<a href="'.JRoute::_('index.php?option=com_jdownloads&amp;Itemid='.$Itemid.'&amp;view=viewcategory&amp;catid='.$catid.'&amp;limitstart='.$limitstart.'&amp;order='.$order.'&amp;dir=asc').'">['.JText::_('JLIST_FE_SORT_ORDER_ASC').'</a>]';
               }    
               $order_bar = JText::_('JLIST_FE_SORT_ORDER_TITLE').' '.$sort_default.$sort_name.$sort_date.$sort_hits.' '.$sort_direction;
    ...
    if ($is_detail) {
        $header = str_replace('{detail_title}', JText::_('JLIST_FRONTEND_SUBTITLE_OVER_DETAIL'), $header); 
    }                
    ...
    if ($is_summary) {
        $header = str_replace('{summary_title}', JText::_('JLIST_FRONTEND_HEADER_SUMMARY_TITLE'), $header); 
    }         
...
function makeFooter($make_back_button, $is_showcats, $is_one_cat, $sum_pages, $limit, $limitstart, $site_aktuell, $pageNav, $is_summary, $is_detail) {
	...
	if ($make_back_button && $jlistConfig['view.back.button']){
        $footer = str_replace('{back_link}', '<a href="javascript:history.go(-1)">'.JText::_('JLIST_FRONTEND_BACK_BUTTON').'</a>', $footer); 
    } else {
...
function buildFieldTitles($html_file, $file){
    ...
    if ($jlistConfig['remove.field.title.when.empty']){
        $html_file = ($file->license) ? str_replace('{license_title}', JText::_('JLIST_FE_DETAILS_LICENSE_TITLE'), $html_file) : str_replace('{license_title}', '', $html_file);
        $html_file = ($file->price) ? str_replace('{price_title}', JText::_('JLIST_FE_DETAILS_PRICE_TITLE'), $html_file) : str_replace('{price_title}', '', $html_file);                                          
        $html_file = ($file->language) ? str_replace('{language_title}', JText::_('JLIST_FE_DETAILS_LANGUAGE_TITLE'), $html_file) : str_replace('{language_title}', '', $html_file);
        $html_file = ($file->size) ? str_replace('{filesize_title}', JText::_('JLIST_FE_DETAILS_FILESIZE_TITLE'), $html_file) : str_replace('{filesize_title}', '', $html_file);
        $html_file = ($file->system) ? str_replace('{system_title}', JText::_('JLIST_FE_DETAILS_SYSTEM_TITLE'), $html_file) : str_replace('{system_title}', '', $html_file);
        $html_file = ($file->author) ? str_replace('{author_title}', JText::_('JLIST_FE_DETAILS_AUTHOR_TITLE'), $html_file) : str_replace('{author_title}', '', $html_file);
        $html_file = ($file->url_home) ? str_replace('{author_url_title}', JText::_('JLIST_FE_DETAILS_AUTHOR_URL_TITLE'), $html_file) : str_replace('{author_url_title}', '', $html_file);
        $html_file = ($file->date_added != '0000-00-00 00:00:00') ? str_replace('{created_date_title}', JText::_('JLIST_FE_DETAILS_CREATED_DATE_TITLE'), $html_file) : str_replace('{created_date_title}', '', $html_file);
        $html_file = ($file->downloads != '') ? str_replace('{hits_title}', JText::_('JLIST_FE_DETAILS_HITS_TITLE'), $html_file) : str_replace('{hits_title}', '', $html_file);
        $html_file = ($file->created_by) ? str_replace('{created_by_title}', JText::_('JLIST_FE_DETAILS_CREATED_BY_TITLE'), $html_file) : str_replace('{created_by_title}', '', $html_file);
        $html_file = ($file->modified_by) ? str_replace('{modified_by_title}', JText::_('JLIST_FE_DETAILS_MODIFIED_BY_TITLE'), $html_file) : str_replace('{modified_by_title}', '', $html_file);
        $html_file = ($file->modified_date != '0000-00-00 00:00:00') ? str_replace('{modified_date_title}', JText::_('JLIST_FE_DETAILS_MODIFIED_DATE_TITLE'), $html_file) : str_replace('{modified_date_title}', '', $html_file);
        $html_file = ($file->file_date != '0000-00-00 00:00:00') ? str_replace('{file_date_title}', JText::_('JLIST_FE_DETAILS_FILE_DATE_TITLE'), $html_file) : str_replace('{file_date_title}', '', $html_file);
        $html_file = ($file->url_download) ? str_replace('{file_name_title}', JText::_('JLIST_FE_DEATAILS_FILE_NAME_TITLE'), $html_file) : str_replace('{file_name_title}', '', $html_file);          
    } else {    
        ...
        $html_file = str_replace('{file_name_title}', JText::_('JLIST_FE_DEATAILS_FILE_NAME_TITLE'), $html_file);   
    }
    return $html_file;
} 
