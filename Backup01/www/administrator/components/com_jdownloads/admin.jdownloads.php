...
function categoriesPublish( $cid=null, $publishform=1,  $option ) {
  ...
  if ($database->loadResult()){
      ...
      $msg = JText::_('JLIST_BACKEND_PUBLISH_CATS_ERROR');
      $mainframe->redirect( 'index2.php?option='.$option.'&task=categories.list', $msg, 'error' );            
  }  
...
function categoriesSave($option,$apply=0){
	...
	if(empty($row->cat_title)) {
        ...
        $mainframe->redirect("index2.php?option=".$option."&task=categories.edit&cid=".$row->cat_id, JText::_('JLIST_BACKEND_CATSEDIT_ERROR_TITLE')." ", 'error');
    }
    ...
    if ($jlistConfig['create.auto.cat.dir'] && $row->cat_title && !$checked_cat_title){
        ...
        $mainframe->redirect("index2.php?option=".$option."&task=categories.edit&cid=".$row->cat_id, JText::_('JLIST_BACKEND_CATSEDIT_ERROR_FOLDER_NAME')." ", 'error');
    }     
    ...
    if ($new_cat || $new_subcat) {
    	...
    	if ($message) {
			...
			$mainframe->redirect("index2.php?option=".$option."&task=categories.edit&cid=".$row->cat_id, $message." ", 'error');
    	}
   ...
   $database->query();    
	...
	if (!$apply) {
		if ($sum_changed != -1){
            ...
            $mainframe->redirect("index2.php?option=".$option."&task=categories.list", JText::_('JLIST_BACKEND_CATSEDIT_SAVE')." ".$msg, 'error');
        }    
	} else {
		if ($sum_changed != -1){
            ...
            $mainframe->redirect("index2.php?option=".$option."&task=categories.edit&hidemainmenu=1&cid=".$row->cat_id, JText::_('JLIST_BACKEND_CATSEDIT_SAVE')." ".$msg, 'error');
        }   
	} 	
}
...
function categoriesEdit($option, $cid){
    ...
    $access_list[] = JHTML::_('select.option', '99', JText::_('JLIST_BACKEND_CATSEDIT_CAT_ACCESS_LEVEL_99'));
    ...
    $access_list[] = JHTML::_('select.option', '02', JText::_('JLIST_BACKEND_CATSEDIT_CAT_ACCESS_LEVEL_02'));    
    ...
    $groups_list[] = JHTML::_('select.option', '0', JText::_('JLIST_BACKEND_CATSEDIT_CAT_GROUPS_ACCESS_0'));
  ...
  . " onchange=\"javascript:if (document.adminForm.cat_pic.options[selectedIndex].value!='') {document.imagelib.src='$cat_pic_dir_path' + document.adminForm.cat_pic.options[selectedIndex].value} else {document.imagelib.src=''}\"", 'value', 'text', $row->cat_pic );
	...
	jlist_HTML::categoriesEdit($option, $row, $inputbox_pic, $access_box, $last_dir_entry, $path_dir_entry, $groups_box);
}
...
function filesPublish( $cid=null, $publishform=1,  $option, $cat_id ) {
  ...
  $database = &JFactory::getDBO();
    ...
    if ($time_frame_files = $database->loadResultArray()){
        ...
        $msg .= ' - '.$sum_timeframe_files .JText::_('JLIST_BACKEND_FILESEDIT_TIMEFRAME_NOT_PUBLISH_MSG')." ";
	...
	if($file['tmp_name'] != ''){
      ...
      if ($only_name != ''){
        ...
        if(@move_uploaded_file($file['tmp_name'], $target_path)) {
		   ...
		   $mainframe->redirect("index2.php?option=".$option."&task=files.edit&hidemainmenu=1&cid=".$row->file_id, JText::_('JLIST_BACKEND_FILESEDIT_CHECK_PERMISSIONS')." ", 'error');
		}
      } else {
           // invalid filename
           $mainframe->redirect("index2.php?option=".$option."&task=files.edit&hidemainmenu=1&cid=".$row->file_id, JText::_('JLIST_BACKEND_FILESEDIT_INVALID_FILENAME')." ", 'error'); 
      }  
...
function filesCopySave($option, $cat_id_act){
    ...
    if ($cat_id){
        ...
        foreach($files as $file){
            ...
            $file->date_added = JHTML::_('date', 'now','%Y-%m-%d %H:%M:%S');
            $database->setQuery("INSERT INTO #__jdownloads_files (`file_id`, `file_title`, `file_alias`, `description`, `description_long`, `file_pic`, `thumbnail`, `thumbnail2`, `thumbnail3`, `price`, `release`, `language`, `system`, `license`, `url_license`, `license_agree`, `update_active`, `size`, `date_added`, `url_download`, `extern_file`, `mirror_1`, `mirror_2`, `url_home`, `author`, `url_author`, `created_by`, `created_mail`, `modified_by`, `modified_date`, `submitted_by`, `custom_field_1`, `custom_field_2`, `custom_field_3`, `custom_field_4`, `custom_field_5`, `custom_field_6`, `custom_field_7`, `custom_field_8`, `custom_field_9`, `custom_field_10`, `custom_field_11`, `custom_field_12`, `custom_field_13`, `custom_field_14`, `downloads`, `cat_id`, `ordering`, `published`, `checked_out`, `checked_out_time`) 
                                                           VALUES (NULL, '$filetitle', '$file->file_alias', '$file->description', '$file->description_long', '$file->file_pic', '$file->thumbnail', '$file->thumbnail2', '$file->thumbnail3', '$file->price', '$file->release', '$file->language', '$file->system', '$file->license', '$file->url_license', $file->license_agree, '$file->update_active', '$filesize', '$file->date_added', '$url_download', '$file->extern_file', '$file->mirror_1', '$file->mirror_2', '$file->url_home', '$file->author', '$file->author_url', '$file->created_by', '$file->created_mail', '', '0000-00-00 00:00:00', $file->submitted_by, '0', '$file->custom_field_1', '$file->custom_field_2', '$file->custom_field_3', '$file->custom_field_4', '$file->custom_field_5', '$file->custom_field_6', '$file->custom_field_7', '$file->custom_field_8', '$file->custom_field_9', '$file->custom_field_10', '$file->custom_field_11', '$file->custom_field_12', '$file->custom_field_13', '$file->custom_field_14', '$cat_id', '0', '$publish_files', '0', '0000-00-00 00:00:00')");
            if (!$database->query()) {
        ...
        foreach ($custom_arr[0] as $custom){
             if ($custom < 6){
               ...
               if ($custom == 1){
                 ...
                 $select_box[0] = JHTML::_('select.option',  0, JText::_('JLIST_BACKEND_FILE_EDIT_SELECT_TITLE'));
               ...
               // create the input fields 
                  ...
                  if ($custom == 13){  
                       if ($jlistConfig['files.editor'] == "1") {
                          $input_field = $editor->display( 'custom_field_13',  @$row->custom_field_13 , '500', '300', '60', '5', true, $params ) ;
                      } else {
                  ...
                  if ($custom == 14){
                      if ($jlistConfig['files.editor'] == "1") {
                          $input_field = $editor->display( 'custom_field_14',  @$row->custom_field_14 , '500', '300', '60', '5', true, $params ) ;
                      } else {  
...
$swfUploadHeadJs ='
        ...
        var settings = 
            ...
            file_types : "*.*",
            file_types_description : "'.JText::_('JLIST_BACKEND_UPLOADER_FILES_SELECT_TITLE').'",
            ...
            //button_text: \'<span class="Font">'. JText::_('JLIST_BACKEND_UPLOADER_FILES_BUTTON_TITLE').'</span>\',
...
function upload(){
   ...
   if ($fileSize >  $limit){
    ...
    if ($only_name != ''){
        ...
        echo JText::_('JLIST_BACKEND_FILESEDIT_INVALID_FILENAME');
        exit(0);
    }    
...
function manageFiles($option, $task, $limitstart){
   global $mainframe, $jlistConfig, $limit;
     ...
     for ($i=0; $i < count($filenames); $i++){
         ...
         $files_info[$i]['date'] = strftime($jlistConfig['global.datetime'], filemtime($files_dir.$filenames[$i]));               
     ...
     if (count($cid)){
         foreach ($cid as $file){
             ...
             if (!JFile::delete(JPATH_ROOT.DS.$jlistConfig['files.uploaddir'].DS.$file)){
                 // can not delete!
                 $mainframe->redirect("index2.php?option=".$option."&task=manage.files", JText::_('JLIST_BACKEND_MANAGE_FILES_DELETE_ERROR'), 'error');
            } else {    
                $deleted++;
            } 
         }
         $msg = sprintf(JText::_('JLIST_BACKEND_MANAGE_FILES_DELETE_SUCCESS'),$deleted);    
     }    
     $mainframe->redirect("index2.php?option=".$option."&task=manage.files", $msg);
}    
...
function listLogs($option, $task, $limitstart){
    ...
    $anonymous = JText::_('JLIST_BACKEND_VIEW_LOGS_GUEST_NAME');
    $where = '';
...
function deleteLogs($option, $cid){
    ...
    $msg = $total.' '.JText::_('JLIST_BACKEND_VIEW_LOGS_DEL_MSG')." ";
    $mainframe->redirect( 'index2.php?option='. $option .'&task=view.logs', $msg );
}
...
function editGroups($option, $cid){
    ...
    $musers = array();
    $toAddUsers = array();
    $blocked = JText::_('JLIST_BACKEND_GROUPS_EDIT_USER_BLOCKED');
    // get members
...
function saveGroups($option, $cid, $apply=0){
  global $mainframe;
    ...
    $database->setQuery("UPDATE #__jdownloads_groups SET groups_members='".$members_ids."' WHERE id=".(int)$row->id);
    $database->query();
    
    if(!$apply) $mainframe->redirect("index2.php?option=".$option."&task=view.groups", JText::_('JLIST_BACKEND_GROUPS_EDIT_SAVED')." ");
    else $mainframe->redirect("index2.php?option=".$option."&task=edit.groups&cid=".$row->id, JText::_('JLIST_BACKEND_GROUPS_EDIT_SAVED')." ");
}
...
function deleteGroups($option, $cid){
    ...
    if ($sum = $database->loadResult() > 0 || in_array($jlistConfig['upload.access.group'], $cid)){
       $msg = JText::_('JLIST_BACKEND_GROUPS_LIST_DEL_ERROR')." ";
        $mainframe->redirect('index2.php?option='.$option.'&task=view.groups', $msg, 'error' );
    }    
    ...
    $msg = $total .JText::_('JLIST_BACKEND_GROUPS_LIST_DEL_MSG')." ";
    $mainframe->redirect('index2.php?option='.$option.'&task=view.groups', $msg );
}
...
function cancelGroups($option){
    ...
    $row = new jlist_groups( $database );
    $row->bind( $_POST );
    $mainframe->redirect("index2.php?option=".$option."&task=view.groups");
}
...
function editTemplatesCats($option, $cid){
	global $mainframe;
    require_once(EL_ADMIN_PATH.DS.'jd_layouts.php');    
	$user = &JFactory::getUser();
	$database = &JFactory::getDBO();
	if(is_array($cid)) $cid = 0;
    ...
    if ($row->template_name == ''){
       $row->cols = 1; 
       $row->template_text = $JLIST_BACKEND_SETTINGS_TEMPLATES_CATS_DEFAULT;
...
function editTemplatesFiles($option, $cid){
    ...
    // load template text default
    if ($row->template_name == ''){
       $row->template_text = $JLIST_BACKEND_SETTINGS_TEMPLATES_FILES_DEFAULT;
...
function editTemplatesDetails($option, $cid){
    ...
    // load template text default
    if ($row->template_name == ''){
       $row->template_text = $JLIST_BACKEND_SETTINGS_TEMPLATES_DETAILS_DEFAULT;
...
function editTemplatesSummary($option, $cid){
    ...
    // load template text default
    if ($row->template_name == ''){
       $row->template_text = $JLIST_BACKEND_SETTINGS_TEMPLATES_SUMMARY_DEFAULT;
...
function showConfig($option){
	...
	$user_groups = array();
	$user_groups[] = JHTML::_('select.option', '99', JText::_('JLIST_BACKEND_CATSEDIT_CAT_ACCESS_LEVEL_99'));
    ...
    $egroups_list[] = JHTML::_('select.option', '0', JText::_('JLIST_BE_CONFIG_EDIT_ACCESS_GROUPS_LIST_SELECT'));
    ...
    $tabs = array();
    $tabs[] = JHTML::_('select.option', '0', JText::_('JLIST_FE_NO'));
    $tabs[] = JHTML::_('select.option', '1', JText::_('JLIST_BACKEND_SET_USE_TABS_BOX_SLIDERS'));
    $tabs[] = JHTML::_('select.option', '2', JText::_('JLIST_BACKEND_SET_USE_TABS_BOX_TABS'));
...
function saveConfig($option){
    ...
    if ($jlistConfig['upload.access.group'] != '0' && $jlistConfig['upload.access'] != '99' || ($jlistConfig['upload.access.group'] == '0' && $jlistConfig['upload.access'] == '99')){
        ...
        $msg .= ' - '.JText::_('JLIST_BACKEND_SET_NOT_VALID_UPLOAD_RIGHTS');
        $error_msg = true;
    } 
    ...
    if ($old_dir != $new_dir) {
      	if ( !@rename( $old_dir, $new_dir ) ) {
		    ...
		    $mainframe->redirect('index2.php?option='.$option.'&task=config.show',$msg, 'error');	
        } else {
   		    // $msg = '';
	    }
    }    
	...
	if ($error_msg){
        $mainframe->redirect('index2.php?option='.$option.'&task=config.show',JText::_('JLIST_BACKEND_SETTINGS_SAVED').$msg, 'error');
    } else {
...
function addIPToBlocklist($option,$cid){
    ...
    $msg = $total.' '.JText::_('JLIST_BACKEND_LOG_LIST_BLOCK_IP_ADDED');
