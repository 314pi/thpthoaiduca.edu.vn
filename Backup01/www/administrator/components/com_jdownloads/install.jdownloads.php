...
function com_install() {
  ...
  define('JD_FRONTEND_PATH',  JPATH_ROOT.DS.'components'.DS.'com_jdownloads');
    ...
    /*  Install plugin  */
    if (JFile::exists(JPATH_ROOT.DS.'plugins'.DS.'system'.DS.'jdownloads_system_plugin.php')){
        JFile::delete(JPATH_ROOT.DS.'plugins'.DS.'system'.DS.'jdownloads_system_plugin.php');
        JFile::delete(JPATH_ROOT.DS.'plugins'.DS.'system'.DS.'jdownloads_system_plugin.xml');
    }    

    JFile::move(
                JD_BACKEND_PATH.DS.'plugins'.DS.'system'.DS.'jdownloads_system_plugin.php',
                JPATH_ROOT.DS.'plugins'.DS.'system'.DS.'jdownloads_system_plugin.php'
    );
    JFile::move(
                JD_BACKEND_PATH.DS.'plugins'.DS.'system'.DS.'jdownloads_system_plugin.xml',
                JPATH_ROOT.DS.'plugins'.DS.'system'.DS.'jdownloads_system_plugin.xml'
    );
    
    JFolder::delete(JD_BACKEND_PATH.DS.'plugins'.DS.'system');
    JFolder::delete(JD_BACKEND_PATH.DS.'plugins');

    $query = 'SELECT `published` FROM `#__plugins` WHERE `element`="jdownloads_system_plugin"';
    ...
    if($result == null){
        ...
        ."\nVALUES"
        ."\n('jDownloads - System Plugin', 'jdownloads_system_plugin', 'system', 0, 0, 1, 0, 0, 0, '0000-00-00 00:00:00', '')";
        $database->setQuery($query);
        $database->query();
    } else {
        $query = 'UPDATE `#__plugins` SET `published` = 1 WHERE `element`="jdownloads_system_plugin"';
        $database->setQuery($query);
        $database->query();
    }
...
// *******************************************************************************************
       ...
       // new alias fields
        ...
        if (!$result[$prefix.'jdownloads_files']['custom_field_1']){
            $database->SetQuery("ALTER table #__jdownloads_files add "
            . " custom_field_1 tinyint(2) NOT NULL default '0' AFTER update_active,"
            . " add custom_field_2 tinyint(2) NOT NULL default '0' AFTER custom_field_1,"
            . " add custom_field_3 tinyint(2) NOT NULL default '0' AFTER custom_field_2,"
            . " add custom_field_4 tinyint(2) NOT NULL default '0' AFTER custom_field_3,"
            . " add custom_field_5 tinyint(2) NOT NULL default '0' AFTER custom_field_4,"
            . " add custom_field_6 varchar(255) NOT NULL default '' AFTER custom_field_5,"
            . " add custom_field_7 varchar(255) NOT NULL default '' AFTER custom_field_6,"
            . " add custom_field_8 varchar(255) NOT NULL default '' AFTER custom_field_7,"
            . " add custom_field_9 varchar(255) NOT NULL default '' AFTER custom_field_8,"
            . " add custom_field_10 varchar(255) NOT NULL default '' AFTER custom_field_9,"
            . " add custom_field_11 date NOT NULL default '0000-00-00' AFTER custom_field_10,"        
            . " add custom_field_12 date NOT NULL default '0000-00-00' AFTER custom_field_11,"                  
            . " add custom_field_13 TEXT NOT NULL default '' AFTER custom_field_12,"                  
            . " add custom_field_14 TEXT NOT NULL default '' AFTER custom_field_13");
            if ($database->query()) {
                $sum_added_fields = $sum_added_fields + 14;        
            }
             if (!$result[$prefix.'jdownloads_files']['license_agree']){
...
// *******************************************************************************************
        ...
        if (!$temp) {
            ...
            $cat_layout = stripslashes($JLIST_BACKEND_SETTINGS_TEMPLATES_CATS_DEFAULT);
            ...
            if($dir_exist) {
                if (is_writable(JPATH_SITE."/jdownloads")) {      
                     if (!is_dir(JPATH_SITE."/jdownloads/".JText::_('JLIST_SAMPLE_DATA_CAT_FOLDER_ROOT'))){
                        // daten speichern
                        // dirs für cats
                        $makdir = JFolder::create(JPATH_SITE."/jdownloads/".JText::_('JLIST_SAMPLE_DATA_CAT_FOLDER_ROOT'), 0755);
                        $makdir = JFolder::create(JPATH_SITE."/jdownloads/".JText::_('JLIST_SAMPLE_DATA_CAT_FOLDER_ROOT').'/'.JText::_('JLIST_SAMPLE_DATA_CAT_FOLDER_SUB'), 0755);
                        // cat erstellen in db
                        if ($makdir) {
